...
spec = do
  ...
  describe "program" $ do
    ...
    it "should parse a union type declaration" $
      parse program "" (L.pack "(declare foo (a b) (union a b) (i1 i2))") `shouldParse`
      [ Dec
          "foo"
          (Forall
             [TV "a", TV "b"]
    ...
    it "should parse a struct type declaration" $
      parse program "" (L.pack "(declare foo (a b) i1 ((struct a b)))") `shouldParse`
      [ Dec
          "foo"
          (Forall
             [TV "a", TV "b"]
             (TArr (TCon "i1") [TPro (TVar (TV "a")) (TVar (TV "b"))]))
      ]
